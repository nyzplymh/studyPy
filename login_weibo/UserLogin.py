...
class WeiBoLogin(object):
    ...
    def login(self, user_name, pass_word):
        ...
        # login weibo.com
        login_url_1 = "http://login.sina.com.cn/sso/login.php?client=ssologin.js(v1.4.5)&_=%d" % int(time.time())
...
if __name__ == "__main__":
    ...
    weibo = WeiBoLogin()
    result = weibo.login("用户名", "密码")
    url = 'http://weibo.com/aj/message/getbyid'
    hasChat = True
    mid = '4096466262012216' # 第一条信息的id 右键查看元素可以看到
    htmls =[]
    while hasChat :
        params = {
            'ajwvr': 6,
            'mid': mid, #消息id
            'count': 199, #每页显示数量
            'uid': '5576276117',#聊天对象的用户ID
            '_t': '0',
            '__rnd': str(time.time())#时间戳
        }

        response = weibo.session.get(url,params = params)
        chatRecord = json.loads( response.text)
        dataStr = json.dumps(chatRecord['data'])
    ...
    newHtmls = list(reversed(htmls))
    file = open("d:\\chatRecord\\chatRecord.html", "w+",encoding='utf-8')
    i = 0
    for html in newHtmls:
        if i%50 == 0:
            print(i ,u"条---------------------------------")
        file.write(html)
        i += 1